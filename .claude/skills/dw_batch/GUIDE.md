...
**Performance impact:** Copy saves **25-40% processing time** (2-5 minutes on typical document batches)

**When to copy:**
- Total file size **< 1GB** OR
- **at least 1 scanned PDF document** OR
- Files on **mounted Windows drives** (`/mnt/c/`, `/mnt/d/`) OR
- Files on **network shares** or **cloud-synced folders**
- If size of relevant files on mounted drive is > 1GB, ask user for consent to go ahead with copying 
...
- **Bash tool timeout:** When running `poll_and_process.py` via Claude Code's Bash tool, set `timeout` to at least **1,200,000 ms (20 min)** for 30B batches and **1,800,000 ms (30 min)** for 235B batches. The default 120s Bash timeout will kill polling before the batch completes. The `completion_window` in config.toml is the API SLA (not the Bash timeout).
