---
name: dw_batch
description: Async batch processing using Doubleword API. Process multiple documents cost-effectively (50-85% cheaper) for analysis, summarization, translation, OCR, and structured data extraction. Non-urgent, background tasks, long-running tasks, token-heavy, token-burning.
---
...
3. **FIRST: Check file location, total size of files, and copy to /tmp if needed:**
   - check the size of the batch (total size of all files) and number of files
   - If files are on mounted drives (/mnt/c/, /mnt/d/, network shares) **AND** size of mounted files < 0.5GB 
   ...
   - if size of mounted files > 0.5GB, ask user if they are happy to copy to /tmp for faster processing (recommended)
   - If you do end up copying files over, tell the user that you copied the files to a temporary location for faster processing and will clean up after
