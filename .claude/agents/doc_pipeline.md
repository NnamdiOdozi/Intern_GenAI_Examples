...
2. **Review is opt-in, not automatic - always confirm with the caller, never silently default.** Check the caller's prompt for an explicit instruction on whether to run `doc_reviewer` (e.g. "with review", "review it too", "no review needed", "skip review"). If explicit, follow it - no need to ask. **If unstated, you must stop and ask the caller** (AskUserQuestion or a direct question if AskUserQuestion isn't available) whether to run `doc_reviewer` before proceeding past this step - do not assume yes or no. Frame the ask around cost: `doc_reviewer` is Opus-based and runs external API cross-checks (CrossRef/arXiv/OpenAlex/Semantic Scholar), making it the most expensive part of this pipeline. Remember the caller's answer - it governs steps 9-10 below.
